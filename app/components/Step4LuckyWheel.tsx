...
export const Step4LuckyWheel = () => {
  ...
  const spinWheel = async () => {
    ...
    // The arrow is at the top (0 degrees or 270 degrees depending on implementation)
    // Assuming 0 degrees is at 3 o'clock for CSS rotate, but we rotated the div. 
    // Let's verify logic:
    // If wheel rotates clockwise, the segment passing the top pointer (at -90deg or 270deg relative to start) changes.
    // Simpler: The pointer is at the TOP.
    // If we rotate X degrees, the slice at TOP is determined by (360 - (X % 360)) % 360
    ...
    <div className="relative w-full h-full flex flex-col items-center justify-center p-8 overflow-hidden">
      ...
      <div className="relative group">
        ...
        >
                ...
                return (
                    <div
                        ...
                        style={{
                            ...
                            transform: `rotate(${rotation}deg) skewY(-30deg)`, // skew to make it a slice? No, conic-gradient is easier but div slicing needs correct skew/clip-path.
                            // Let's use conic-gradient for background and absolute divs for text.
                            // Actually, simpler CSS implementation for slices:
                            // Use clip-path or just rotate lines.
                            // Let's try a different approach: Conic gradient for colors, then overlay text.
                            display: 'none' // Hiding this naive attempt
                        }}
                    >
                    </div>
                )
            ...
            {PRIZES.map((prize, i) => {
                ...
                return (
                    ...
                    >
                        <span className="absolute top-[12%] left-1/2 -translate-x-1/2 text-white font-bold text-lg md:text-2xl writing-vertical-rl select-none" style={{ writingMode: 'vertical-rl' }}>
                            {prize.label}
                        </span>
                    </div>
                )
        ...
        <button
            onClick={spinWheel}
            disabled={isSpinning}
            className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-16 h-16 bg-white rounded-full shadow-lg border-4 border-yellow-500 flex items-center justify-center font-bold text-red-500 hover:scale-110 transition-transform z-30 disabled:opacity-80 disabled:hover:scale-100"
        >
            {isSpinning ? '...' : 'GO'}
        </button>
      </div>
      ...
      <AnimatePresence>
        {result && (
          <motion.div
            ...
            exit={{ opacity: 0 }}
            className="mt-8 bg-white/60 backdrop-blur-md p-6 rounded-2xl border border-white/40 text-center max-w-sm shadow-xl"
          >
